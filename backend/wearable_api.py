"""Normalized wearable-device ingestion API for Aida."""

from __future__ import annotations

import uuid
from datetime import datetime, timezone
from typing import Any, Dict, List, Literal, Optional

from fastapi import APIRouter, Depends, HTTPException
from pydantic import BaseModel, Field

from healthkit_api import HealthSample


def _now() -> datetime:
    return datetime.now(timezone.utc)


WearableProvider = Literal[
    "health_connect",
    "samsung_health",
    "fitbit",
    "garmin",
    "oura",
]


class WearableSyncRequest(BaseModel):
    profile_id: str
    provider: WearableProvider
    device_name: Optional[str] = None
    device_model: Optional[str] = None
    os_version: Optional[str] = None
    samples: List[HealthSample] = Field(default_factory=list, max_length=5000)


class WearableSyncResponse(BaseModel):
    ok: bool = True
    provider: WearableProvider
    inserted: int
    skipped: int
    last_sync_at: datetime


def build_wearable_router(db, auth) -> APIRouter:
    router = APIRouter(prefix="/api/health/wearables", tags=["wearables"])

    async def require_access(account_id: str, profile_id: str, write: bool = False):
        if not await auth.has_profile_access(account_id, profile_id, write=write):
            raise HTTPException(404, "Profile not found")

    @router.post("/sync", response_model=WearableSyncResponse)
    async def sync_wearable(
        data: WearableSyncRequest,
        account: Dict[str, Any] = Depends(auth.require_account),
    ):
        await require_access(str(account["id"]), data.profile_id, write=True)
        source = f"wearable:{data.provider}"
        synced_at = _now()
        inserted = 0
        skipped = 0

        for sample in data.samples:
            if sample.external_id:
                existing = await db.vitals.find_one(
                    {
                        "profile_id": data.profile_id,
                        "source": source,
                        "external_id": sample.external_id,
                    },
                    {"_id": 0},
                )
                if existing:
                    skipped += 1
                    continue

            doc = {
                "id": str(uuid.uuid4()),
                "profile_id": data.profile_id,
                "source": source,
                "provider": data.provider,
                "external_id": sample.external_id,
                "metric": sample.metric,
                "type": sample.metric,
                "value": sample.value,
                "unit": sample.unit,
                "start_at": sample.start_at,
                "end_at": sample.end_at or sample.start_at,
                "source_name": sample.source_name,
                "device_name": sample.device_name or data.device_name,
                "device_model": data.device_model,
                "os_version": data.os_version,
                "metadata": sample.metadata,
                "synced_at": synced_at,
            }
            await db.vitals.insert_one(doc)
            inserted += 1

        return WearableSyncResponse(
            provider=data.provider,
            inserted=inserted,
            skipped=skipped,
            last_sync_at=synced_at,
        )

    @router.get("/status/{profile_id}")
    async def wearable_status(
        profile_id: str,
        account: Dict[str, Any] = Depends(auth.require_account),
    ):
        await require_access(str(account["id"]), profile_id)
        rows = await (
            db.vitals.find(
                {"profile_id": profile_id},
                {"_id": 0},
            )
            .sort("synced_at", -1)
            .to_list(500)
        )

        providers: Dict[str, Dict[str, Any]] = {}
        for row in rows:
            source = str(row.get("source") or "")
            if not source.startswith("wearable:"):
                continue
            provider = source.split(":", 1)[1]
            if provider in providers:
                continue
            providers[provider] = {
                "connected": True,
                "last_sync_at": row.get("synced_at"),
                "device": {
                    "name": row.get("device_name"),
                    "model": row.get("device_model"),
                    "os_version": row.get("os_version"),
                },
            }

        return {"providers": providers}

    @router.get("/latest/{profile_id}/{provider}")
    async def latest_wearable(
        profile_id: str,
        provider: WearableProvider,
        limit: int = 100,
        account: Dict[str, Any] = Depends(auth.require_account),
    ):
        await require_access(str(account["id"]), profile_id)
        limit = max(1, min(limit, 500))
        return await (
            db.vitals.find(
                {
                    "profile_id": profile_id,
                    "source": f"wearable:{provider}",
                },
                {"_id": 0},
            )
            .sort("start_at", -1)
            .to_list(limit)
        )

    return router
