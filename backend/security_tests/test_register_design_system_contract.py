...
def test_registration_keyboard_flow_and_autofill_contract():
    ...
    assert 'onSubmitEditing={continueWithEmail}' in source
...
def test_registration_social_methods_share_locked_visual_language():
    ...
    assert 'testID="register-social-options"' in source
...
def test_registration_keeps_design_lock_geometry():
    ...
    assert 'input: { height: 52' in source
    assert 'primary: { minHeight: 52' in source
    assert 'socialButton: { minHeight: 50' in source
