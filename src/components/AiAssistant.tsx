...
export const AiAssistant: React.FC<AiAssistantProps> = ({
  ...
  const hasAnyData = documents.length > 0 || dailyLogs.length > 0 || diaryEntries.length > 0 || pressureLogs.length > 0;
  ...
  const [messages, setMessages] = useState<Message[]>([
    {
      ...
      sender: 'ai',
      text: hasAnyData
        ? `Похоже, самое время сделать паузу и позаботиться о себе. Я Аида — твой персональный помощник. Я изучила твои последние записи, показатели сна, давление и дневник. Задавай любой вопрос, и мы вместе во всём разберёмся 🤍`
        : `Привет! Я Аида — твой персональный помощник по здоровью. Пока в твоём профиле нет записей, поэтому я ещё ничего не анализировала — начни с чек-ина, дневника или загрузки анализа, и я помогу разобраться в данных 🤍`,
