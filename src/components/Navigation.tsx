...
export const Navigation: React.FC<NavigationProps> = ({
  ...
  const navTabs = React.useMemo(
    () => [
      ...
      { id: 'aida', label: 'Чат', icon: MessageSquare },
  ...
  const activeIndex = React.useMemo(() => {
    ...
    if (['body_map', 'pressure_diary', 'mental_diary'].includes(currentScreen)) return 1;
    if (currentScreen === 'reminders') return 2;
            ...
            {navTabs.map((tab, idx) => {
              ...
              return (
                ...
                >
                  ...
                  <span className="nav-label text-[11px] font-medium tracking-tight">{tab.label}</span>
                </button>
              );
            })}
          </nav>
