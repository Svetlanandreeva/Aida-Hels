  ...
  return (
    ...
    >
      <View style={s.progressRow}>
        <Text style={s.eyebrow}>AIDA · 3/3</Text>
        <View
          style={s.progress}
          accessibilityRole="progressbar"
          accessibilityLabel={ru ? "Прогресс настройки Аиды" : "Aida setup progress"}
          accessibilityValue={{ min: 0, max: 3, now: 3, text: ru ? "Шаг 3 из 3" : "Step 3 of 3" }}
        >
          <View accessible={false} style={[s.progressFill, { width: "100%" }]} />
        </View>
      </View>
      ...
      <View style={s.actions}>
        ...
        <Pressable style={[s.primary, (!canSave || busy) && { opacity: .55 }]} onPress={finish} disabled={!canSave || busy} testID="finish-lifestyle-onboarding" accessibilityRole="button" accessibilityLabel={ru ? "Завершить настройку" : "Finish setup"} accessibilityState={{ disabled: !canSave || busy, busy }}>{busy ? <ActivityIndicator color={colors.onSurfaceInverse} /> : <Text style={s.primaryText}>{ru ? "Завершить настройку" : "Finish setup"}</Text>}</Pressable>
      </View>
