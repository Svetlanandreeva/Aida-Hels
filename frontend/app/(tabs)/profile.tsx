    ...
    <View style={styles.container}>
      ...
      >
        <View style={[styles.cover, { paddingTop: insets.top }]}>
          <LinearGradient colors={[figma.coverA, figma.coverB, figma.coverC]} start={{ x: 0.1, y: 0 }} end={{ x: 0.9, y: 1 }} style={StyleSheet.absoluteFill} />
        ...
        <View style={styles.avatarWrap}><Image source={{ uri: avatarFor(activeProfile.kind) }} style={styles.avatar} contentFit="cover" /></View>
        ...
        {nutritionEnabled ? (
          <Pressable style={styles.medCard} onPress={() => router.push("/nutrition" as any)} testID="nutrition-profile-link">
            <View style={styles.medIcon}><Ionicons name="restaurant-outline" size={23} color={figma.text} /></View>
            <View style={styles.flex}>
              <Text style={styles.rowTitle}>{lang === "ru" ? "Питание" : "Nutrition"}</Text>
              <Text style={styles.rowSubtitle}>{lang === "ru" ? "Дневник питания и анализ рациона" : "Food diary and diet analysis"}</Text>
            </View>
            <Ionicons name="chevron-forward" size={20} color={figma.muted} />
          </Pressable>
        ) : null}
        ...
        <View style={styles.settingsCard} testID="settings-card">
          ...
          <View style={styles.settingRow}>
            ...
            <Pressable onPress={() => setLang(lang === "ru" ? "en" : "ru")} testID="profile-language-row"><Text style={styles.settingValue}>{lang === "ru" ? "RU / EN" : "EN / RU"}</Text></Pressable>
          </View>
          ...
          <Pressable style={styles.settingRow} onPress={() => void toggleNutrition()} disabled={nutritionSaving} testID="nutrition-setting-toggle">
            ...
            <View style={[styles.moduleChip, nutritionEnabled && styles.moduleChipActive]}>
              {nutritionSaving ? <ActivityIndicator size="small" color={nutritionEnabled ? "#FFFFFF" : figma.text} /> : <Text style={[styles.moduleChipText, nutritionEnabled && styles.moduleChipTextActive]}>{nutritionEnabled ? (lang === "ru" ? "Вкл" : "On") : (lang === "ru" ? "Выкл" : "Off")}</Text>}
            </View>
          </Pressable>
          <View style={styles.divider} />
          <Pressable style={styles.settingRow} onPress={openEdit} testID="edit-profile-button"><Text style={styles.settingLabel}>{t("edit")}</Text><Ionicons name="chevron-forward" size={18} color={figma.muted} /></Pressable>
        </View>

        <Pressable style={styles.secondaryRow} onPress={() => router.push("/settings" as any)} testID="full-settings-link">
          <Ionicons name="settings-outline" size={20} color={figma.text} /><Text style={styles.secondaryText}>{lang === "ru" ? "Все настройки и разделы профиля" : "All profile settings"}</Text><Ionicons name="chevron-forward" size={18} color={figma.muted} />
        </Pressable>
      </ScrollView>
...
const EditField: React.FC<{ label: string; children: React.ReactNode }> = ({ label, children }) => <View style={styles.editField}><Text style={styles.fieldLabel}>{label}</Text>{children}</View>;
...
const styles = StyleSheet.create({
  ...
  cover: { height: 180, overflow: "hidden" }, coverTexture: { ...StyleSheet.absoluteFillObject, opacity: 0.08 },
  ...
  langText: { color: figma.text, fontSize: 12, fontWeight: "800" }, avatarWrap: { alignItems: "center", marginTop: -44 },
  ...
  metaRow: { flexDirection: "row", justifyContent: "center", gap: 8, marginTop: 4 }, pill: { height: 28, minWidth: 48, paddingHorizontal: 12, borderRadius: 999, backgroundColor: figma.card, alignItems: "center", justifyContent: "center" }, pillText: { color: figma.text, fontSize: 12, fontWeight: "800" },
  metricsRow: { flexDirection: "row", gap: 11, paddingHorizontal: 16, marginTop: 16 }, metric: { flex: 1, height: 82, borderRadius: 18, backgroundColor: figma.card, alignItems: "center", justifyContent: "center" }, metricValue: { color: figma.text, fontSize: 16, fontWeight: "800" }, metricLabel: { color: figma.muted, fontSize: 11, marginTop: 5 },
  medCard: { minHeight: 72, marginHorizontal: 16, marginTop: 16, paddingHorizontal: 16, borderRadius: 26, backgroundColor: figma.card, flexDirection: "row", alignItems: "center", gap: 14 }, medIcon: { width: 42, height: 42, borderRadius: 21, backgroundColor: "#F0F0EE", alignItems: "center", justifyContent: "center" }, flex: { flex: 1 }, rowTitle: { color: figma.text, fontSize: 15, fontWeight: "800" }, rowSubtitle: { color: figma.muted, fontSize: 11, lineHeight: 16, marginTop: 2 },
  allergyCard: { minHeight: 84, marginHorizontal: 16, marginTop: 16, paddingHorizontal: 24, paddingVertical: 14, borderRadius: 26, backgroundColor: figma.card }, allergyTitle: { color: figma.text, fontSize: 13, fontWeight: "800" }, allergyRow: { flexDirection: "row", flexWrap: "wrap", gap: 8, marginTop: 12 }, allergyPill: { height: 24, paddingHorizontal: 12, borderRadius: 999, backgroundColor: figma.allergy, alignItems: "center", justifyContent: "center" }, allergyText: { color: figma.text, fontSize: 11, fontWeight: "700" }, noneText: { color: figma.muted, fontSize: 12 },
  settingsCard: { marginHorizontal: 16, marginTop: 16, paddingHorizontal: 24, paddingTop: 14, paddingBottom: 6, borderRadius: 26, backgroundColor: figma.card }, settingsTitle: { color: figma.text, fontSize: 14, fontWeight: "800", marginBottom: 6 }, settingRow: { minHeight: 48, flexDirection: "row", alignItems: "center", justifyContent: "space-between", gap: 12 }, settingLabel: { color: figma.text, fontSize: 13 }, settingValue: { color: figma.muted, fontSize: 12 }, divider: { height: 1, backgroundColor: figma.divider },
  settingActionLabel: { flex: 1, flexDirection: "row", alignItems: "center", gap: 10 }, settingHint: { color: figma.muted, fontSize: 10, lineHeight: 14, marginTop: 2 }, moduleChip: { minWidth: 48, height: 28, paddingHorizontal: 10, borderRadius: 999, backgroundColor: "#EFEFED", alignItems: "center", justifyContent: "center" }, moduleChipActive: { backgroundColor: figma.text }, moduleChipText: { color: figma.muted, fontSize: 11, fontWeight: "800" }, moduleChipTextActive: { color: "#FFFFFF" },
  secondaryRow: { marginHorizontal: 16, marginTop: 14, minHeight: 56, paddingHorizontal: 18, borderRadius: 22, backgroundColor: "rgba(251,251,250,0.66)", flexDirection: "row", alignItems: "center", gap: 12 }, secondaryText: { flex: 1, color: figma.text, fontSize: 13, fontWeight: "700" }, editTitle: { color: figma.text, fontSize: 24, fontWeight: "800", marginBottom: 18 }, editField: { marginBottom: 16 }, fieldLabel: { color: figma.muted, fontSize: 13, marginBottom: 8, fontWeight: "600" }, input: { height: 52, borderRadius: 16, paddingHorizontal: 16, backgroundColor: figma.card, color: figma.text, borderWidth: 1, borderColor: "#D8D8D6", fontSize: 16 },
});
