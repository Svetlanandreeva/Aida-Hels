...
export default function WomensHealthScreen() {
  ...
  const save = async () => {
    ...
    try {
      await api.updateProfile(activeId, { women_health: payload });
      await reload();
  ...
  return <ScrollView style={s.page} contentContainerStyle={[s.content,{paddingTop:insets.top+20,paddingBottom:insets.bottom+36}]}>
    ...
    {mode==="planning"&&<Card title={ru?"Планирование беременности":"Pregnancy planning"}><Field label={ru?"Планируем с":"Planning since"} value={planningSince} onChangeText={setPlanningSince} placeholder="YYYY-MM-DD"/><Text style={s.note}>{ru?"Аида использует этот контекст только там, где он релевантен, и не трактует его как подтверждённую беременность.":"Aida uses this context only where relevant and does not treat it as a confirmed pregnancy."}</Text></Card>}
    ...
    {mode==="off"&&<Card title={ru?"Модуль выключен":"Module off"}><Text style={s.note}>{ru ? "Данные женского здоровья не используются в аналитике, пока вы снова не выберете нужный режим." : "Women's health data is not used for analytics until you choose a mode again."}</Text></Card>}
