...
const GOALS = [
  ["general", "Общее здоровье", "General health"], ["labs", "Анализы", "Labs"], ["sleep", "Сон", "Sleep"],
  ["mental", "Психика", "Mental wellbeing"], ["meds", "Лекарства", "Medications"], ["women", "Женское здоровье", "Women's health"],
];
...
export default function OnboardingScreen() {
  ...
  const toggleGoal = (goal: string) => setGoals((current) => current.includes(goal) ? current.filter((g) => g !== goal) : [...current, goal]);
  const finish = async (skipDetails = false) => {
    ...
    try {
      ...
      await api.updateProfile(activeId, {
        ...
        height_cm: skipDetails ? activeProfile?.height_cm : (height ? Number(height.replace(",", ".")) : null),
        weight_kg: skipDetails ? activeProfile?.weight_kg : (weight ? Number(weight.replace(",", ".")) : null),
        goals: skipDetails ? (activeProfile?.goals || []) : goals,
    ...
    } catch { setError(ru ? "Не удалось сохранить профиль" : "Could not save profile"); }
  ...
  return <ScrollView style={styles.page} contentContainerStyle={[styles.content, { paddingTop: insets.top + 28, paddingBottom: insets.bottom + 36 }]} keyboardShouldPersistTaps="handled">
    ...
    <Section title={ru ? "Основное" : "Basics"}>
      <Input label={ru ? "Имя *" : "Name *"} value={name} onChangeText={setName} placeholder={ru ? "Как к вам обращаться" : "Your name"} />
      <Input label={ru ? "Дата рождения" : "Date of birth"} value={dob} onChangeText={setDob} placeholder="YYYY-MM-DD" />
      <Text style={styles.label}>{ru ? "Пол / медицинский контекст" : "Sex / medical context"}</Text>
      <View style={styles.row}>{[["female", ru ? "Женский" : "Female"], ["male", ru ? "Мужской" : "Male"], ["", ru ? "Не указывать" : "Prefer not to say"]].map(([v,l]) => <Pressable key={l} style={[styles.chip, sex === v && styles.chipActive]} onPress={() => setSex(v)}><Text style={[styles.chipText, sex === v && styles.chipTextActive]}>{l}</Text></Pressable>)}</View>
      <View style={styles.two}><View style={styles.half}><Input label={ru ? "Рост, см" : "Height, cm"} value={height} onChangeText={setHeight} placeholder="168" keyboardType="decimal-pad" /></View><View style={styles.half}><Input label={ru ? "Вес, кг" : "Weight, kg"} value={weight} onChangeText={setWeight} placeholder="65" keyboardType="decimal-pad" /></View></View>
    </Section>
